...
def generate_characters(config):
    """Place the chosen character in the world based on config."""
    character = config.get("character", "cat")
    if character == "dog":
        return '<use href="#dog" x="400" y="204" />'
    return '<use href="#cat" x="400" y="210" />'
...
def generate_svg():
    ...
    chars = generate_characters(config)
    ...
    with open(OUTPUT_FILE, "w") as f:
        f.write(content)
    print(f"Generated {OUTPUT_FILE} with weather: {weather}, character: {config.get('character', 'cat')}")
